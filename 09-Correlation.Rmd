...
| **Lectures**    | 9.1: Frequency and count data (13:19 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=c1d88524-703a-4d9c-8acf-b262012515d7))     |
|                 | 9.2: Chi-squared goodness of fit test (16:34 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=22a0833e-5130-4f4c-a3e7-b2620124ff48))                      |
|                 | 9.3: Chi-squared test of association (16:03 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=0d02d907-1297-4535-86e7-b2620124fdf0))     |
|                 | 9.4: Correlation key concepts (7:02 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=a06d2f8d-cd32-400c-8192-b2620124f5ba))     |
|                 | 9.5: Correlation underlying mathematics (12:06 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=0ea2d074-65b6-4121-a4a4-b2620124f480))                        |
|                 | 9.6: Correlation hypothesis testing (27:28 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=59cf95e3-8473-47cf-ad30-b26201246367))                        |
| **Lecture**     | Lecture: 19 MAR 2025 (WED) 09:00-10:00 Cottrell LT B4 |
| **Practical**   | Analysis of counts and correlations ([Chapter 31](#Chapter_31)) |
|                 | Room: Cottrell 2A15                                    |
...
| **Assessments** | [Week 9 Practice quiz](https://canvas.stir.ac.uk/courses/17581/quizzes/40228) on Canvas                     |


Week 9 introduces count data and correlations, and how to interpret scatterplots.

[Chapter 29](#Chapter_29) focuses on count data. 
Count data are defined by natural numbers of discrete entities (e.g., the number of trees or birds in a site). 
We can test statistical hypotheses on count data using a Chi-square (chi-square) distribution, which is introduced in the first part of the chapter. 
...
A simple example with real numbers introduces the Chi-square, followed by goodness of fit tests and tests of association using player decision-making data originally introduced in [Chapter 17](#Chapter_17).

[Chapter 30](#Chapter_30) focuses on continuous data. 
...
This test of the correlation coefficient relies on the t-distribution, which was introduced in Chapter 19. 
Example data for this chapter include morphological measurements from two species of non-pollinating fig wasps. 
From these data, the chapter demonstraates how to calculate the Pearson product-moment correlation coefficient and the Spearman's rank correlation coefficient. 
Jamovi output for a correlation matrix is shown with an explanation of how to correctly interpret it.
	
[Chapter 31](#Chapter_31) demonstrates how to use jamovi to run and correctly interpret output for the Chi-square tests introduced in Chapter 29 and correlation tests introduced in Chapter 30. 
...
The first exercise works through a very simple goodness of fit test by hand, demonstrating the calculations needed to obtain a Chi-square test statistic. 
Jamovi is then used to recreate the hand calculations, and this is followed by another goodness of fit exercise and a test of association between bumblebee colony and survival. 
How to make scatterplots is then illustrated in jamovi using bumblebee mass and carbon dioxide output. 
Exercises then demonstrate out to calculate Pearson product moment correlation coefficients and Spearman rank correlation coefficients in jamovi. 
The chapter ends with an example of how to run a goodness of fit test in a simplified way with data that are not in a tidy format.



# Frequency and count data {#Chapter_29}

In this book, we have introduced hypothesis testing as a tool to determine if variables were sampled from a population with a specific mean (one sample t-test in [Section 22.1](#one-sample-t-test)), or if different groups of variables were sampled from a population with the same mean (the independent samples t-test in [Section 22.2](#independent-samples-t-test) and ANOVA in [Chapter 24](#Chapter_24)). 
...
That is, the dependent variables of the t-test and ANOVA could always, at least in theory, take any real value (i.e., any decimal). 
...
[Chapter 15](#Chapter_15) included some examples of count data when introducing probability distributions (e.g., counts of heads or tails in coin flips, or the number of people testing positive for COVID-19). 
...
We could test this null hypothesis using a Chi-square goodness of fit test ([Section 29.1](#the-chi-square-distribution)). 
...
We could test this kind of hypothesis using a Chi-square test of association.
...
The general motivation for introducing the Chi-square distribution is the same as it was for the t-distribution ([Chapter 19](#Chapter_19)) or F-distribution ([Section 24.1](#the-f-distribution)). 
...
First, we can take another look at the standard normal distribution from [Section 15.4.4](#normal-distribution) (Figure 15.9).
Suppose that we randomly sampled four values from the standard normal distribution.
...
Note that $\chi^{2}$ cannot be negative because when we square a number that is either positive or negative, we always end up with a positive value (e.g., $-2^{2} = 4$, see [Section 1.1](#numbers-and-operations)). 
...
And if we are sampling randomly from the normal distribution, we should not expect to get the same $\chi^{2}$ value from four new random standard normal deviates. 
...
```{r, echo = FALSE, fig.alt = "Plot of a curved distribution that rapidly increases to a maximum at around 2 then slowly decreases.", fig.cap = "A Chi-square distribution that is the expected sum of four squared standard normal deviates, i.e., the sum of four values sampled from a standard normal distribution and squared."}
par(mar = c(5, 5, 1.5, 1.5));
xx        <- seq(from = 0, to = 20, by = 0.001);
pr_chisq  <- dchisq(x = xx, df = 4);
plot(x = xx, y = pr_chisq, type = "l", ylim = c(0, 0.22), 
     ylab = "Probability density",  xlab = "Chi-square value", lwd = 3, xlim = c(0, 20),
...
The shape of the $\chi^{2}$ distribution[^chi_square_shape] is therefore determined by the number of values sampled ($n$), or more specifically the degrees of freedom ($df$, or sometimes $v$), which in a sample is $df = n - 1$. 
This is the same idea as the t-distribution from [Chapter 19](#Chapter_19), which also changed shape depending on the degrees of freedom. 
Figure 29.2 shows the different $\chi^{2}$ probability density distributions for different degrees of freedom.

[^chi_square_shape]: A random variable $X$ has a $\chi^{2}$ distribution if and only if its probability density function is defined by [@Miller2004], $$f(x) = \left\{\begin{array}{ll}\frac{1}{2^{\frac{2}{v}}\Gamma\left(\frac{v}{2}\right)}x^{\frac{v-2}{2}}e^{-\frac{x}{2}} & \quad for\:x > 0 \\ 0 & \quad elsewhere \end{array}\right.$$ In this equation, $v$ is the degrees of freedom of the distribution.

```{r, echo = FALSE, fig.alt = "A plot is shown with three different curve lines, which show three different Chi-square distributions with different degrees of freedom.", fig.cap = "Probability density functions for three different Chi-square distributions, each of which has different degrees of freedom ($df$)."}
df   <- 4;
...
As with the F-distribution from [Section 24.1](#the-f-distribution), visualising the $\chi^{2}$ distribution is easier using an interactive application[^shiny_chi_square_distribution].
...
After all, counts are integer values, and the $\chi^{2}$ distribution is clearly continuous (unlike, e.g., the binomial or Poisson distributions from [Section 15.4](##probability-distributions)). 
...
[^shiny_chi_square_distribution]: https://bradduthie.github.io/stats/app/chi-square/
...
Recall the practical in [Chapter 17](#Chapter_17), in which players of the mobile app game 'Power Up!' chose a small, medium, or large dam at the start of the game. 
...
That is, for our expected counts of each dam size ($E_{\mathrm{size}}$), we expect 20 small ($E_{\mathrm{small}} = 20$), 20 medium ($E_{\mathrm{medium}} = 20$), and 20 large ($E_{\mathrm{large}} = 20$) dams in total (because $60/3 = 20$).
...
The expected counts might still be a bit different from the observed counts of each dam size ($O_{\mathrm{size}}$). 
Suppose, for example, we find that out of our total 60 policy-makers, we observe 18 small ($O_{\mathrm{small}} = 18$), 24 medium ($O_{\mathrm{medium}} = 24$), and 18 large ($O_{\mathrm{large}} = 18$), dams were actually chosen by game players.
...
To get our test statistic[^X_vs_Chi], we now just need to take each observed count, subtract the expected count, square this difference, divide by the expected count, then add everything up,
...
```{r, echo = FALSE, fig.alt = "Plot of curved distribution that starts around 0.5 and decreases, asymptoting toward 0.", fig.cap = "A Chi-square distribution with 2 degrees of freedom, used in a goodness of fit test to determine if all players of the mobile game 'Power Up!' have the same preference for initial dam size. An arrow indicates a $\\chi^{2}$ value of 1.2 on the x-axis, and grey shading indicates the probability of a more extreme value under the null hypothesis."}
chis  <- 1.2
p_val <- 0.5488;
...
For example, we could have tested the null hypothesis that twice as many policy-makers would choose large dams (i.e., $E_{\mathrm{large}} = 40$, $E_{\mathrm{medium}} = 10$, and $E_{\mathrm{small}} = 10$).
...
[^X_vs_Chi]: A lot of statisticians will use $X^{2}$ to represent the test statistic here instead of $\chi^{2}$ [@Sokal1995]. The difference is the upper case 'X' versus the Greek letter Chi, '$\chi$'. The X is used since the test statistic we calculate here is not *technically* from the $\chi^{2}$ distribution, just an approximation. We will not worry about the distinction here, and to avoid confusion, we will just go with $\chi^{2}$. 

## Chi-square test of association

The second kind of test that we will consider for count data is the Chi-square test of association.
While the goodness of fit test focused on a single categorical variable (dam sizes in the example above), the Chi-square test of association focuses on two different categorical variables.
What we want to know is whether or not the two categorical variables are independent of one another [@Box1978].
...
We can again make use of the [Chapter 17](#Chapter_17) game 'Power Up!'.
...
pander::pander(power_table, format = "simple", table.envir = "table",
      caption = "**TABLE 29.1** Counts ($N = 60$) from a mobile game called 'Power Up!', in which players are confronted with trade-offs between energy output, energy justice, and biodiversity. Players can use one of two categories of Operating System (Android or macOS) and build one of three types of dam in the game (Small, Medium, or Large).");
```
...
With the goodness of fit test in [Section 29.2](#chi-squared-goodness-of-fit), we just assumed that all categories were equally likely (i.e., the probability of choosing each size dam was the same). 
...
We want to know if they are *associated* with one another.
...
pander::pander(power_table2, format = "simple", table.envir = "table",
      caption = "**TABLE 29.2** Counts ($N = 60$) from a mobile game called 'Power Up!', in which players are confronted with trade-offs between energy output, energy justice, and biodiversity. Players can use one of two categories of Operating System (Android or macOS) and build one of three categories of dam in the game (Small, Medium, or Large). Outer rows and columns show the probabilities of categories being selected.");
```
...
If these probabilities combine independently[^marginal_probabilities], then we can multiply them to find the probability of a particular combination of categories.
For example, the probability of a player using Android is 0.5 and choosing a small dam is 0.3, so the probability of a player having both Android **and** a small dam is $0.5 \times 0.3 = 0.15$ (see [Chapter 16](#Chapter_16) for an introduction to probability models).
...
colnames(power_table2) <- c("Small", "Medium", "Large", "Probability");
pander::pander(power_table2, format = "simple", table.envir = "table",
      caption = "**TABLE 29.3** Probabilities for each combination of categorical variables from a dataset in which players on either an Android or macOS choose a dam size in the mobile app game 'Power Up!', assuming that variables are independent of one another.");
```
...
pander::pander(power_table3, format = "simple", table.envir = "table",
      caption = "**TABLE 29.4** Expected counts for each combination of categorical variables from a dataset in which players on either an Android or macOS choose a dam size in the mobile app game 'Power Up!', assuming that variables are independent of one another.");
```

We now have both the observed (Table 29.2) and expected (Table 29.4) counts (remember that the expected counts do not *need* to be integers).
To get our $\chi^{2}$ test statistic, we use the same formula as in [Section 29.2](#chi-squared-goodness-of-fit),
...
We can compare this to the null $\chi^{2}$ distribution as we did in the [Section 29.2](#chi-squared-goodness-of-fit) goodness of fit test, but we need to know the correct degrees of freedom.
The correct degrees of freedom[^chi_square_df] is the number of categories in variable 1 ($n_{1}$) minus 1, times the number of categories in variable 2 ($n_{2}$) minus 1,

$$df = (n_{1} - 1) \times (n_{2} - 1).$$

In our example, the degrees of freedom equals the number of dam types minus 1 ($n_{\mathrm{dam}} = 3 - 1)$ times the number of operating systems minus 1 ($n_{\mathrm{OS}} = 2 - 1$).
The correct degrees of freedom is therefore $df = 2 \times 1 = 2$.
We now just need to find the p-value for a Chi-square distribution with two degrees of freedom and a test statistic of $\chi^{2} = 4.889$.
...
Jamovi will calculate the $\chi^{2}$ value and get the p-value for the appropriate degrees of freedom [@Jamovi2022].
To do this, it is necessary to input the categorical data (e.g., Android, macOS) in a tidy format, which will be a focus of [Chapter 31](#Chapter_31).
...
pander::pander(power_table4, format = "simple", table.envir = "table",
      caption = "**TABLE 29.5** Observed counts for each combination of categorical variables from a dataset in which players on either an Android or macOS choose a dam size in the mobile app game 'Power Up!'. The last row and column shows the sum of observed dam sizes and OS users, respectively.");
```
...
[^marginal_probabilities]: We can call these the 'marginal probabilities'.

[^chi_square_df]: This formula works due to a bit of a mathematical trick [@Sokal1995]. The actual logic of the degrees of freedom is a bit more involved. From our total of $k = 6$ different combinations, we actually need to subtract one degree of freedom for the total sample size ($N = 60$), then a degree of freedom for each variable probability estimated (i.e., subtract $n_{1} - 1$ and $n_{2} - 1$ because we need this many degrees of freedom to get the $n_{1}$ and $n_{2}$ probabilities, respectively; if we have all but one probability, then we know the last probability because the probabilities must sum to 1). Since we lose $n_{1} - 1$ and $n_{2} - 1$ degrees of freedom, and 1 for the sample size, this results in $df = k - (n_{1} - 1) - (n_{2} - 1) - 1$. In the case of the 'Power Up!' example, we get $df = 6 - (3 - 1) - (2 - 1) - 1 = 2$. The $df = (n_{1} - 1) \times (n_{2} - 1)$ formulation is possible because $k = n_{1} \times n_{2}$ [@Sokal1995].


# Correlation {#Chapter_30}
...
It is similar to the [Chi-square test of association](#chi-squared-test-of-association) from [Chapter 29](#Chapter_29) in the sense that it is about how variables are associated.
The focus of the [Chi-square test of association](#chi-squared-test-of-association) was on the association when data were categorical (e.g., 'Android' or 'macOS' operating system).
...
Consider two fig wasp species of the genus *Heterandrium* (Figure 30.1).

```{r, echo = FALSE, fig.alt = "Two small black iridescent wasps are shown side by side with wings facing upwards.", fig.cap = "Fig wasps from two different species, (A) Het1 and (B) Het2. Wasps were collected from Baja, Mexico. Image modified from Duthie, Abbott, and Nason (2015).", out.width="100%"}
...
We might want to collect morphological measurements of fig wasp head, thorax, and abdomen lengths in these two species [@Duthie2015b].
...
het_for_table           <- heterandrium;
het_for_table[,2:4]     <- format(het_for_table[,2:4], nsmall = 3);
colnames(heterandrium)  <- c("Species", "Head", "Thorax", "Abdomen");
colnames(het_for_table) <- c("Species", "Head", "Thorax", "Abdomen");
pander::pander(het_for_table, format = "simple", table.envir = "table",
      caption = "**TABLE 30.1** Body segment length measurements (mm) from fig wasps of two species. Data were collected from Baja, Mexico.");
...
```{r, echo = FALSE, fig.alt = "A plot is shown with 11 points, such that points that are higher on the x-axis are generally higher on the y-axis.", fig.cap = "Example of a scatterplot in which fig wasp thorax length (x-axis) is plotted against fig wasp abdomen length (y-axis). Each point is a different fig wasp. Wasps were collected in 2010 in Baja, Mexico."}
par(mar = c(5, 5, 1.5, 1.5));
...
We represent this number with the letter '$r$', which can range from values of -1 to 1.[^corr_range]
...
```{r, echo = FALSE, fig.alt = "Four scatter plots are shown in a 2 by 2 grid with different correlations between x and y variables.", fig.cap = "Examples of scatterplots with different correlation coefficients (r) between two variables (x and y).", fig.height = 5, fig.width = 5}
...
We will look at two types of correlation coefficient, the Pearson product moment correlation coefficient and Spearman's rank correlation coefficient.
The two are basically the same; Spearman's rank correlation coefficient is just a correlation of the ranks of values instead of the actual values.

### Pearson product moment correlation coefficient

To understand the correlation coefficient, we need to first understand covariance.
[Section 12.3](#the-variance) introduced the variance ($s^{2}$) as a measure of spread in some variable $x$,
...
To get the Pearson product moment correlation coefficient[^pearson_corr] ($r$), we need to standardise the covariance so that the minimum possible value of $r$ is $-1$ and the maximum possible value of $r$ is 1.
...
Jamovi will make these calculations for us [@Jamovi2022].
...
squared_table           <- data.frame(Row, format(Thorax, nsmall = 3), 
                                      format(Abdomen, nsmall = 3), 
                                      Squared_Deviation);
colnames(squared_table) <- c("Row (i)", "Thorax", "Abdomen", "Squared Deviation");
pander::pander(squared_table, format = "simple", table.envir = "table",
      caption = "**TABLE 30.2** Measurements of 11 fig wasp thorax and abdomen lengths (mm). The fourth column shows the product of the deviations of each measurement from the mean, where mean thorax length is 0.770 and mean abdomen length is 1.210.");
```
...
We will test whether or not this value of $r$ is statistically significant in [Section 30.3](#correlation-hypothesis-testing), but first we will introduce the Spearman's rank correlation coefficient.

[^corr_range]: Note that $r$ is the sample correlation coefficient, which is an estimate of the population correlation coefficient. The population correlation coefficient is represented by the Greek letter '$\rho$' ('rho'), and sometimes the sample correlation coefficient is represented as $\hat{\rho}$.

[^pearson_corr]: We can usually just call this the 'correlation coefficient'.

### Spearman's rank correlation coefficient

We have seen how the *ranks* of data can be substituted in place of the actual values.
This has been useful when data violate the assumptions of a statistical test, and we need a non-parametric test instead (e.g., [Wilcoxon signed rank test](#wilcoxon-test-1), [Mann-Whitney U test](#mann-whitney-u-test-1), or [Kruskal-Wallis H test](#Chapter_26)).
...
To calculate Spearman's rank correlation coefficient for the fig wasp example in [the previous section](#pearson-product-moment-correlation-coefficient), we just need to rank the thorax and abdomen lengths from 1 to 11, then calculate $r$ using the rank values instead of the actual measurements of length.
...
rank_table              <- data.frame(Row, format(Thorax, nsmall = 3), 
                                      Thorax_rank, format(Abdomen, nsmall = 3),
                                      Abdomen_rank);
colnames(rank_table)    <- c("Wasp (i)", "Thorax", "Thorax Rank", "Abdomen", 
                             "Abdomen Rank");
pander::pander(rank_table, format = "simple", table.envir = "table",
      caption = "**TABLE 30.3** Measurements of 11 fig wasp thorax and abdomen lengths (mm) and their ranks.");
```
...
To get Spearman's rank correlation coefficient, we just calculate $r$ using the ranks.
...
$$r_{\mathrm{rank}(x),\mathrm{rank}(y)} = \frac{6.4}{3.317 \times 3.317}$$
...
The reason why we might want to use Spearman's rank correlation coefficient instead of the Pearson product moment correlation coefficient is explained in the next section.
...
As it turns out, the sample correlation coefficient ($r$) will be approximately t-distributed around a true correlation ($\rho$) with a t-score defined by $r - \rho$ divided by its standard error ($\mathrm{SE}(r)$)[^se_of_corr],

$$t = \frac{r - \rho}{\mathrm{SE}(r)}.$$

Since our null hypothesis is that variables are uncorrelated, $\rho = 0$.
Jamovi will use this equation to test whether or not the correlation coefficient is significantly different from zero [@Jamovi2022].
The reason for presenting it here is to show the conceptual link to other hypothesis tests in earlier chapters.
In [Section 22.1](#one-sample-t-test), we saw that the one sample t-test defined $t$ as the deviation of the sample mean from the true mean, divided by the standard error.
...
Since our p-value is less than 0.05, we can reject the null hypothesis that fig wasp thorax and abdomen lengths are not correlated (an interactive application[^shiny_corr] can make this more intuitive).

[^se_of_corr]: We can calculate the standard error of $r$ as [@Rahman1968], $$\mathrm{SE}(r) = \sqrt{\frac{1 - r^{2}}{N - 2}}.$$ But this is not necessary to ever do by hand. Note that we lose two degrees of freedom ($N - 2$), one for calculating each variable mean.

[^shiny_corr]: https://bradduthie.github.io/stats/app/corr_click/


# *Practical*. Analysis of counts and correlations {#Chapter_31}

This chapter focuses on applying the concepts from [Chapter 29](#Chapter_29) and [Chapter 30](#Chapter_30) in jamovi [@Jamovi2022].
Exercises in this chapter will use the [Chi-square goodness of fit](#chi-squared-goodness-of-fit) test, the [Chi-square test of association](#chi-squared-test-of-association),
and the [correlation coefficient](#correlation-hypothesis-testing).
For all of these examples, this chapter will use a dataset inspired by @Burrows2022.
...
Survived ($E_{\mathrm{surv}}$): _________________

Died ($E_{\mathrm{died}}$): ______________
...
We did this once in [Chapter 17](#Chapter_17) for calculating probabilities.
...
Survived ($O_{\mathrm{surv}}$): _________________

Died ($O_{\mathrm{died}}$): ______________


Try to use the formula in [Section 29.2](#chi-squared-goodness-of-fit) to calculate the $\chi^{2}$ test statistic.
Here is what it should look like for the two counts in this dataset,

$$\chi^{2} = \frac{(O_{\mathrm{surv}} - E_{\mathrm{surv}})^{2}}{E_{\mathrm{surv}}} + \frac{(O_{\mathrm{died}} - E_{\mathrm{died}})^{2}}{E_{\mathrm{died}}}.$$
...
In the jamovi Analyses tab, select 'Frequencies' from the toolbar, then select 'N Outcomes: $\chi^{2}$ Goodness of fit' from the pull-down menu.
...
Note that the distrACTION module in jamovi includes a $\chi^{2}$ distribution (called 'x2-Distribution'), which you can use to compute probabilities and quantiles in the same way we did for previous distributions in the book.
...
```{r, echo = FALSE, fig.alt = "Jamovi interface is shown with boxes of variables to be used for a Chi-square test of association with colony in a 'Rows' box and survived in a 'columns' box.", fig.cap = "Jamovi interface for running a Chi-square test of association on bumblebee survival versus colony in a dataset.", out.width="100%"}
knitr::include_graphics("img/jamovi_test_of_association_interface.png");
```

There is a pull-down called 'Statistics' below the Contingency Tables input.
...
Suppose that we want to test if bumblebee mass at the start of the experiment (mass) is associated with carbon dioxide output (CO~2~).
...
Put mass on the x-axis and CO~2~ on the y-axis, as shown in Figure 31.3.

```{r, echo = FALSE, fig.alt = "Jamovi interface is shown with a variable called 'mass' placed in a box called 'X-Axis', and a variable called 'CO_2' placed in a box called 'Y-axis'.", fig.cap = "Jamovi interface for building a scatterplot with bumblebee mass on the x-axis and carbon dioxide output on the y-axis.", out.width="100%"}
...
Just looking at the scatterplot, does it appear as though bee mass and CO~2~ output are correlated?
Why or why not?
...
Now we can test whether or not bee mass and CO~2~ output are negatively correlated.
...
Remember from [Section 30.3](#correlation-hypothesis-testing) that a test of the Pearson product moment correlation assumes that the sample $r$ is normally distributed around the true correlation coefficient.
If both of our variables (mass and CO~2~) are normally distributed, then we can be confident that this assumption will not be violated.
...
To test if mass and CO~2~ are normally distributed, navigate to the Descriptives panel in jamovi (where we usually find the summary statistics of variables).
Place mass and CO~2~ in the 'Variables' box, then scroll down and notice that there is a checkbox under **Normality** for 'Shapiro-Wilk'.
...
To run the correlation coefficient test, choose the button in the jamovi toolbar called 'Regression', then select the first option 'Correlation Matrix' from the pull-down menu.
...
Remember that we are interested in the alternative hypothesis that mass and CO~2~ are negatively correlated, so we should select the radio button 'Correlated negatively'.
...
Suppose that we had used the Pearson product moment correlation coefficient instead of Spearman's rank correlation coefficient.
...
pander::pander(count_tab, format = "simple", table.envir = "table",
      caption = "**TABLE 31.1** Counts of bees that did not survive (No) or did survive (Yes) in an experiment involving radiation.");
```
...
```{r, echo = FALSE, fig.alt = "Jamovi spreadsheet showing just 2 rows and 2 columns as 'Survived' and 'Count'.", fig.cap = "Jamovi data frame with a simple organisation of count data.", out.width="100%"}
...
pander::pander(assoc_tab, format = "simple", table.envir = "table",
      caption = "**TABLE 31.2** Counts of bees that did not survive (No) or did survive (Yes) for different levels of radiation.");
```

If we put Table 31.2 into jamovi, we can run a $\chi^{2}$ test of association by navigating to the 'Frequencies' button in the jamovi toolbar and selecting 'Independent Samples: $\chi^{2}$ test of association' from the pull-down.
...
[^bumblebee_data]: [https://bradduthie.github.io/stats/data/bumblebee.csv](https://bradduthie.github.io/stats/data/bumblebee.csv)
