...
Consequently, these tools are indispensable in the sciences, and their importance in research continues to grow. 
By developing your statistical understanding, you will become better able to critically evaluate the scientific literature and conduct your own scientific research. 
In this module, you will learn important skills for working with environmental, ecological, and biological data sets. 
...
As the name implies, these ILOs define the core learning outcomes from the module.
...
During the semester, we will highlight the relevance of these skills using real examples from biological and environmental sciences and use readings and software that are free and open access (and therefore accessible to you even after the module and your degree has completed).
...
The font size, type, and colour scheme should also be adjustable.
...
- **Practical Group A**: WED 13:05-15:55 in Cottrell 2A17
- **Practical Group B**: THU 09:05-11:55 in Cottrell 2A17
- **Optional Help**: FRI 15:05-17:55 in Cottrell 1A13

These are all of the scheduled face-to-face sessions, with the exception of one at the very end of the semester.
See the [full schedule](#schedule) for all of the specific dates, times, and locations.
...
For each weak, links to all chapters, lectures, and quizzes are available in this book (e.g., the [Week 1 Overview](#Week1)) and the [Learning and Teaching](https://canvas.stir.ac.uk/courses/13075/pages/learning-and-teaching) page on Canvas.


### Book chapters {-#book_chapters}

The book chapters that you need to read for each are all listed at the start of each section in the book (e.g., [Week 1](#Week1)) and in each week's [Learning and teaching content](https://canvas.stir.ac.uk/courses/13075/pages/learning-and-teaching) on Canvas.
...
Reading the [book online](bradduthie.github.io/SCIU4T4/) gives you the option of adjusting the text size (small or large), font (serif or sans serif), and background (white, sepia, or night).
...
An eBook is development, but the equations do not render well.
...
This book is the only one that you **need** to read to do well in this module, but each week also includes some readings that are recommend, optional, or advanced.
...
Advanced readings go beyond the taught material and are sometimes quite technical.
...
Wherever possible, readings are free and open access (always the case for required and recommended readings), are from inexpensive books.
References in this book are also useful sources of information, but a lot of these references are from expensive statistics texts.
Most of these texts are available in the library, and all of them are owned by the module coordinator (happily shared upon request).
...
Similarly, weekly links to readings, lectures, practicals, and assessments are posted at the start of the sections of this book and in the [Learning and Teaching](https://canvas.stir.ac.uk/courses/13075/pages/learning-and-teaching) content on Canvas.
You will also receive weekly announcements at 08:00 on Monday summarising what needs to be done each week (again, with relevant links).
Our objective here is to provide a clear structure for the module so that you always know what is happening and when.

Some aspects of the module must be completed on Canvas.
All of the links below are also on the main Canvas page.

- **[Discussions](https://canvas.stir.ac.uk/courses/13075/discussion_topics)** are a place where you can ask questions about the module. Each week, a new topic will be introduced so that you can ask questions pertaining to that week (we are not strict about which week you post in; this is more just to help keep everything organised). You can also ask questions anonymously on the **[SCIU4T4 Padlet](https://padlet.com/alexanderduthie/ox0i6vbgakpvb47b)**. It is completely fine to ask for practice problems in the discussion boards, but please try to be as specific as possible about what you want to practice (this helps us come up with a good question).
- **[Quizzes](https://canvas.stir.ac.uk/courses/13075/quizzes)** are where all practice quizzes and assessments will be located. You must take these on Canvas, and you will need a laptop or desktop computer to complete them.
- **[People](https://canvas.stir.ac.uk/courses/13075/users)** is where you can sign up for a practical group (A or B).
...
Table: Summary of module assessments, whether they are practice (formative) or for a grade (summative), how much they count for the final grade, the weeks of material that the test includes, and the time of the test.

All tests and exams are taken online on Canvas.
You can access the assessments in table above using the '[Quizzes](https://canvas.stir.ac.uk/courses/13075/quizzes)' link on canvas, or access an assessment directly with the above links.

Tests and exams are completed online.
Tests and exams are open book and open note.
You are free to use any learning materials throughout the module, but you **must complete your test independently**.
In other words, you cannot confer with anyone else about the test material while sitting it (except for module instructors if you have question).
...
To complete tests and exams, you will need to run statistical analyses on a laptop or desktop computer (a mobile phone will not work, or at least be extremely inconvenient to the extent that you might not be able to complete the assessment on time).
...
However, extremely high grades (80+) are indended to indicate truly exceptional work that goes above and beyond what is required to pass the module:
...
It is entirely fine to have a look at the datasets before the test starts and get comfortable in your test environment.
...
Note that because all assessments are tests or exams, it is not possible to apply for an extension request as you would for an assignment (i.e., tests and exams cannot be taken late except under extenuating circumstances).
...
Module demonstrators include postgraduate researchers who have experience in data analysis from their own PhD work.
...
While practicals are not summatively assessed, the skills that you learn during module practicals are absolutely critical for being able to correctly answer questions on the tests and exams.
Practicals exist to provide a low-pressure environment for learning these skills, so we hope that you can take your time and focus on completing them with the support of your instructors and classmates.
If you finish early, then you are welcome to continue exploring the practical topic and asking questions, but you are not required to stay the full 2 hours and 50 minutes.
...
This year, and for the foreseeable future, we are using [Jamovi](https://www.jamovi.org/).
...
- It is lightweight, and much less prone to crashing than other statistical software.
...
You can also run Jamovi [from a browser](https://www.jamovi.org/cloud.html), but there are currently some time limitations on doing this (sessions are limited to 45 minutes).
...
Sessions are run online using MS Teams and do not require any active participation (contact the module coordinator if you want to be included in the group).
...
This book was entirely written in R using software called "Rmarkdown".


## Timetable {-#timetable}

```{r, echo = FALSE, fig.cap = "Dates, times, and locations of all synchronous activities in Statistical Techniques for Spring 2023."}
