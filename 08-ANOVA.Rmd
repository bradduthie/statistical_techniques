...
Jamovi is quite minimalist for a one-way ANOVA, but these 4 statistics (F, df1, df2, and p) are all that is really needed.
...
```{r, echo = FALSE, fig.alt = "Jamovi output is shown with a small table that includes 4 rows and 5 two-way ANOVA.", fig.cap = "Jamovi output table for a two-way ANOVA, which tests for the effects of species, tree, and their interaction on fig wasp wing lengths. Species wing length measurements were collected in 2010 near La Paz in Baja, Mexico.", out.width="100%"}
...
Figure 26.2 shows the these results visually.
...
Note that we *do not* need to apply any kind of correction for multiple comparisons (see [Chapter 24](#Chapter_24)) when testing these 3 different hypotheses.
This is because
