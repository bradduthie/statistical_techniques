...
And the number of combinations groups exponentially[^41] with each new group added to the dataset (Table 21.2)
...
In other words, about 40\% of the time, we will conclude that at least 2 species differ in their mean wing lengths[^42], even when all species *really do* have the same wing length.
...
If we reject $H_{0}$, then we can start comparing group means (more on this in [Chapter 25](#Chapter_25)).
...
For example, if we wanted to test whether or not a coin is fair, then we could flip it 10 times and compare the number of times it comes up heads to probabilities of the binomial distribution given $P(Heads) = 0.5$ and $N = 10$ (see [Chapter 14.4.1](#binomial-distribution) Figure 14.5).
To test variances, we will calculate the ratio of variances (F), then compare it to the F probability density function[^43].
The ratio of the variances for samples 1 and 2 is [@Sokal1995],
...
[^43]: The F-distribution was originally discovered in the context of the ratio of random variables with chi-square distributions, with each variable being divided by its own degree of freedom [@Miller2004]. We will look at the Chi-squar distribution in [Week 9](#Week9).
...
The SS was introduced when showing how the variance is calculated in [Chapter 12.3](#the-variance),
...
For example, for Het1, the squared deviation would be $4(`r wing_lengths_means[1]` - `r grand_mean_wing_l`)^{2}$ because there are 4 flies, we multiply the squared deviation from the mean by 4.
...
Calculating the above across the 5 species of wasps gives a value of `r ms_among`. 
...
If we sum up the sum of squared deviations in Table 23.5, we get a $SS_{within} =$ `r sum(SS_species)`.
...
From [Chapter 23.2.1](#anova-mean-variance-among-groups), we now have our mean variance among groups,

$$MS_{among} = `r ms_among/4`.$$

From [Chapter 23.2.2](#anova-mean-variance-among-groups), we now have our mean variance within groups,
...
The area shaded in grey in Figure 23.2, where F > `r (ms_among/4) / (sum(SS_species)/15)`, is approximately $P =$ `r p_val`.
This is our p-value.
...
The difference could instead be attributable to wasps being sampled from different trees (more on this in [Chapter 26](#Chapter_26)).
...
We can test this assumption using a Levene's test in the same way that we did for the independent samples t-test in [Chapter 22](#independent-samples-t-test-1).
If we reject the null hypothesis that groups have the same mean, then we should potentially consider a non-parametric alternative test such as the Kruskall-Wallis H test (see [Chapter 25](#Chapter_25)).
...
In [Chapter 23.2](#one-way-anova), we rejected the null hypothesis that all fig wasp species have the same mean wing length.
...
The equation is included here only to show the similarity between the calculation of H and the calculating of F in [Chapter 23.2](#one-way-anova).
...
We can then compare H to a suitable null distribution, which might be calculated using an appropriate algorithm [e.g., @Choi2003].
Some statistical programs such as Jamovi and R compare H to a Chi-square distribution (see [Week 9](#Week9)), which is an effective approximation [@Miller2004].
...
```{r, echo = FALSE}
wing_lengths_tree <- read.csv("data/site_96_wing_lengths_tidy_tree.csv");
knitr::kable(wing_lengths_tree, format = "simple", table.envir = "table",
      caption = "Wing lengths (mm) measured for 5 unnamed species of non-pollinating fig wasps collected from 2 fig trees in 2010 near La Paz in Baja, Mexico.");
```
...
This section will not delve into the mathematics of the two-way ANOVA[^47].
...
[^47]: For those interested, chapter 11 of @Sokal1995 works all of this out step by step. 
