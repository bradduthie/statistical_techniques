...
Note that we are using $\mu$ for the mean here instead of $\bar{x}$, and $\sigma$ for the standard deviation instead of $s$, to differentiate between the sample estimates of [Chapter 11](#Chapter_11) and [Chapter 12](#Chapter_12).
...
In [Chapter 15](#Chapter_15), we will get an even closer look at the normal distribution and why it is especially useful.
...
We can therefore ask about the *distribution* of these sample means.
...
The CLT allows us to make inferences about the means of non-normally distributed distributions [@Sokal1995], to create confidence intervals around estimates sample means, and to apply statistical hypothesis tests that would otherwise not be possible.
...
We could then use a statistical program such as Jamovi, our [interactive application](https://bradduthie.shinyapps.io/zandp/), or an old-fashioned z-table[^18] to find that only about 2\% of values are expected to be higher than $x = 9.1$ in our normally distributed data.
