...
In this case, our trees, land patches, or soil samples are our units of **observation**.
...
Tidy datasets that can be used in statistical analysis are defined by three characteristics [@Wickham2014].
...
Table: Hypothetical tidy data set in which each column of data is a variable and each row of data is an observational unit (tree).
...
# _Practical_: Preparing data {#Chapter_3}
...
knitr::kable(wasps_cross_table, format = "simple", table.envir = "table",
             caption = "An efficienty but untidy way to present count data. Counts of different species of fig wasps (rows) in 5 different fig fruits (columns). Data were originally collected from Baja, Mexico in 2010.");
```
