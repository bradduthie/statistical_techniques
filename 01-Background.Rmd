...
|                 |                                   |
|-----------------|-----------------------------------|
| **Dates**       | 20 January 2025 - 24 January 2025 |
...
| **Lectures**    | 1.0: Numbers and operations (17:48 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=fd875a7f-f60c-43c6-a339-b26201193422)) |
|                 | 1.1: Logarithms (6:06 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=39bc09e8-18a6-4521-8398-b26201191920))       |
|                 | 1.2: Order of Operations (7:00 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=72ecf2fc-adb3-4ef3-b450-b2620118acff))     |
|                 | 1.3: Tidy data (13:39 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=e02a1f5a-53c4-4991-986a-b26201189ae9))  |
|                 | 1.4: Data files (12:15 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=d88edc55-1afb-4b57-b8d2-b26201188a61))    |
| **Lecture**     | Introduction: 22 JAN 2025 (WED) 09:00-10:00 Cottrell LT B4 |
| **Practical**   | Preparing data ([Chapter 3](#Chapter_3))  |
|                 |   Room: Cottrell 2A15         |
|                 |   Group A: 22 JAN 2025 (WED) 10:00-13:00 |
|                 |   Group B: 23 JAN 2025 (THU) 15:00-18:00 |
| **Help hours**  | Brad Duthie                     |
|                 |   Room: Cottrell 2Y8           |
|                 |   24 JAN 2025 (FRI) 14:00-16:00 |
| **Assessments** | [Week 1 Practice quiz](https://canvas.stir.ac.uk/courses/17581/quizzes/40237) on Canvas  |
|                 |                                 |


Week 1 focuses on background mathematics and data organisation.

[Chapter 1](#Chapter_1) reviews some background mathematics that is relevant to introductory statistics. 
...
[Chapter 2](#Chapter_2) focuses entirely on data organisation. 
...
[Chapter 3](#Chapter_3) provides some practical skills for organising datasets and preparing them for statistical analysis. 
The aim of this chapter is for the reader to learn how to take data recorded in the field, laboratory, or some other source and put it into a format that can be used in jamovi. 
Four exercises use data on fig seed counts, fig wasp egg counts, fig wasp counts, and fig wasp morphological measurements from the Sonoran Desert Rock Fig (*Ficus petiolaris*) in Baja, Mexico. 
...
Fortunately, while the *theory* underlying statistical techniques does rely on some quite advanced mathematics [e.g., see @Mclean1991; @Rencher2000; @Miller2004], the *application* of standard statistical tools usually does not.
...
This chapter will review mathematical operations and the symbols used to communicate them.
...
We know that the number 6 is greater than 5, and we express this mathematically as the **inequality**, $6 > 5$.
...
Euler's number $e$ is an irrational number (like $\pi$), which corresponds to the intrinsic rate of increase of a population's size in ecology [@Gotelli2001], or, in banking, interest compounded continuously (like $\pi$, $e$ actually shows up in a lot of different places throughout science and mathematics).
...
```{r, echo = FALSE, fig.alt = "Two panels are shown, the first with a woman handling a bird nest in a field setting, and the second with the same woman handling nest components over a tray in the lab.", fig.cap = "Dr Becky Boulton collects data from nest boxes in the field (A), then she processes nest material in the lab (B).", out.width="100%"}
...
This chapter introduces the tidy format [@Wickham2014], which is widely used for structuring data files.
This chapter will provide an example of how to put data into a tidy format, and how to save a dataset into a file that can be read and used in jamovi [@Jamovi2022].
...
In this book, we will use LibreOffice because it is completely free and open source, and easily available to [download](https://www.libreoffice.org/download/download-libreoffice/)  (http://libreoffice.org).
Excel and [Google Sheets](https://docs.google.com/spreadsheets) are also completely fine to use.

```{r, echo = FALSE, fig.alt = "A spreadsheet is shown with data collected from different trees being presented with different observations spanning columns and rows.", fig.cap = "LibreOffice spreadsheet showing data from fig fruits collected in 2010. Each row is a unique sample (fruit), and columns record properties of the fruit.", out.width="100%"}
...
We will look more at how to interact with the spreadsheet in [Chapter 3](#Chapter_3), but for now we will focus on how the data are organised.
...
For example, we might make one or more measurements on a tree, a patch of land, or in a sample of soil. 
...
Table: **TABLE 2.1** Hypothetical tidy dataset in which each column of data is a variable, and each row of data is an observational unit (tree).
...
The information will therefore always look *exactly* the same, and it can be easily recovered by any text editor, even after years pass and old software inevitably becomes obsolete.
...
For example, the file 'HW9.scx' in Figure 2.4B would have the path name '/home/brad/Spring_2006/Biostatistics/HW9.scx'.
Each folder is contained within slashes, and the file name itself is after the last slash.

```{r, echo = FALSE, fig.alt = "Screenshot showing a directory with five folders of various class names on the left panel. In the right panel, there is a second screenshot showing the inside of one of the four folders with files and other folders.", fig.cap = "File directory of a computer showing (A) the file organisation of classes taken during spring 2006. Within one folder (B), there are multiple sub-folders and files associated with a biostatistics class.", out.width="100%"}
...
Figure 2.5 shows the same folder 'Spring_2006' saved in a different location, on OneDrive.

```{r, echo = FALSE, fig.alt = "Screenshot showing a OneDrive directory with five folders of various class names on the left panel.", fig.cap = "OneDrive file directory showing the file organisation of classes taken during spring 2006.", out.width="100%"}
...
```{r, echo = FALSE, fig.alt = "Screenshot showing a Windows directory with five folders of various class names on the left panel.", fig.cap = "Windows file directory showing the file organisation of classes taken during spring 2006. In this case, the 'Spring\\_2006' folder is located on the desktop; the path to the folder is visible in the toolbar above the folders.", out.width="100%"}
...
In this chapter, we will use a spreadsheet to organise datasets following the tidy approach explained in [Chapter 2](#Chapter_2), then save these datasets as CSV files to be opened in jamovi statistical software [@Jamovi2022].
The data organisation in this chapter can be completed using [LibreOffice Calc](https://www.libreoffice.org/discover/calc/), MS Excel, or [Google Sheets](https://docs.google.com/spreadsheets/).
The screenshots below will be from LibreOffice Calc, but the instructions provided will work on any of the three aforementioned spreadsheet programs.
...
Exercise 3.2 presents some data on the number of eggs produced by five different fig wasp species (more on these in [Chapter 8](#Chapter_8)).
...
Exercise 3.1 focuses on data collected from the fruits of fig trees collected from Baja, Mexico in 2010 [@Duthie2015b; @Duthie2016].
...
```{r, echo = FALSE, fig.alt = "A large tree is shown in the desert with a person dressed in tan clothes in the foreground", fig.cap = "Fully grown Sonoran Desert Rock Fig in the desert of Baja, Mexico.", out.width="100%"}
...
```{r, echo = FALSE, fig.alt = "A zoomed-in portion of a lab notebook is shown, which includes handwritten data in pencil observations of mature fig fruits, including date, species, site, tree, and fruit dimensions.", fig.cap = "Portion of a lab notebook used to record measurements of fig fruits from different trees in 2010.", out.width="100%"}
...
```{r, echo = FALSE, fig.alt = "A screenshot of a spreadsheet with 7 columns and 6 rows of data on fig fruit dimensions.", fig.cap = "Spreadsheet with data organised in a tidy format and nearly ready for analysis.", out.width="100%"}
...
**Site 70, Tree 70, Fruit 1: 238 total**

```{r, echo = FALSE, comment = NA}
if( knitr:::is_latex_output() ) {
  x <- 1;
} else {
    tallies <- knitr::asis_output("\U168E");
    tallone <- knitr::asis_output("\U1D369");
    talltwo <- knitr::asis_output("\U1D36A");
    tallthr <- knitr::asis_output("\U1D36B");
    tallfou <- knitr::asis_output("\U1D36C");
    c1 <- c(rep(tallies, times = 20));
    c2 <- c(rep(tallies, times = 20));
    c3 <- c(rep(tallies, times = 7), tallthr);
   cat(c(c1, "\n", c2, "\n", c3));
}
```
...
**Site 70, Tree 70, Fruit 2: 198 total**
...
if( knitr:::is_latex_output() ) {
  x <- 1;
    ...
    tallfou <- knitr::asis_output("\U1D36C");
    c1 <- c(rep(tallies, times = 20));
    c2 <- c(rep(tallies, times = 19), tallthr);
   cat(c(c1, "\n", c2));
}
```
...
**Site 70, Tree 70, Fruit 3: 220 total**
...
**Site 70, Tree 70, Fruit 4: 169 total**
...
if( knitr:::is_latex_output() ) {
  x <- 1;
    ...
    tallfou <- knitr::asis_output("\U1D36C");
    c1 <- c(rep(tallies, times = 20));
    c2 <- c(rep(tallies, times = 13), tallfou);
   cat(c(c1, "\n", c2));
}
```
...
**Site 70, Tree 70, Fruit 5: 188 total**
...
if( knitr:::is_latex_output() ) {
  x <- 1;
    ...
    tallfou <- knitr::asis_output("\U1D36C");
    c1 <- c(rep(tallies, times = 20));
    c2 <- c(rep(tallies, times = 17), tallthr);
   cat(c(c1, "\n", c2));
}
```
...
**Site 70, Tree 70, Fruit 6: 139 total**
...
if( knitr:::is_latex_output() ) {
  x <- 1;
    ...
    tallfou <- knitr::asis_output("\U1D36C");
    c1 <- c(rep(tallies, times = 20));
    c2 <- c(rep(tallies, times = 7), tallfou);
   cat(c(c1, "\n", c2));
}
```
...
If you are using LibreOffice Calc or MS Excel on your computer, then you should be able to simply save your file as something like 'Fig_fruits.csv', and the program will recognise that you intend to save as a CSV file (in MS Excel, you might need to find the pull-down box for 'Save as type:' under the 'File name:' box and choose 'CSV').
If you are using Google Sheets, you can navigate in the toolbar to 'File $\to$ Download $\to$ Comma-separated values (.csv)', which will start a download of your spreadsheet in CSV format.
...
It will therefore be necessary to save as an XLSX, then convert to CSV later in another spreadsheet program (local version of MS Excel, LibreOffice Calc, or Google Sheets).
...
## Making spreadsheet data tidy

Exercise 3.2 is more self-guided than Exercise 3.1.
After reading [Chapter 2](#Chapter_2) and completing Exercise 3.1, you should have a bit more confidence in organising data in a tidy format.
...
You can download this dataset online[^wasp_egg_data] or recreate it from Table 3.1.
...
Table: **TABLE 3.1** Untidy dataset of egg loads from fig wasps of five different species, including two unnamed species of the genus *Heterandrium* (Het1 and Het2) and three unnamed species of the genus *Idarnes* (LO1, SO1, and SO2).

Using what you have learnt in [Chapter 2](#Chapter_2) and Exercise 3.1, create a tidy version of the wasp egg loads dataset.
...
[^wasp_egg_data]: [https://bradduthie.github.io/stats/data/wasp_egg_loads_untidy.xlsx](https://bradduthie.github.io/stats/data/wasp_egg_loads_untidy.xlsx)

## Making data tidy again
...
pander::pander(wasps_cross_table, format = "simple", table.envir = "table",
             caption = "**TABLE 3.2**  Efficient but untidy way to present count data. Counts of different species of fig wasps (rows) are from five different fig fruits (columns). Data were originally collected from Baja, Mexico in 2010.");
```
...
## Tidy data and spreadsheet calculations

Exercise 3.4 requires some restructuring and calculations.
The dataset that will be used in this exercise includes morphological measurements from five species of fig wasps, the same species used in Exercises 3.2 and 3.3.
The dataset for this exercise can be downloaded online[^wasp_morphology_data].
This dataset is about as untidy as it gets. 
First note that there are multiple sheets in the spreadsheet, which is not allowed in a CSV file.
...
You can click on all of the different tabs to see the measurements of head length, head width, thorax length, thorax width, abdomen length, and abdomen width for wasps of each of the five species.
All of the measurements are collected in millimetres.
...
$$V_{\mathrm{head}} = \frac{4}{3}\pi \left(\frac{Head_L + Head_W}{4}\right)^{3}.$$

In the equation above, $Head_{\mathrm{L}}$ is head length (mm) and $Head_{\mathrm{W}}$ is head width (note, $(Head_\mathrm{L} + Head_\mathrm{W})/4$ estimates the radius of the head).
...
To calculate $V_{\mathrm{head}}$ for the first wasp in row 2 of the spreadsheet, we could select the spreadsheet cell H2 and type the code, `=(4/3)*(3.14)*((B2+C2)/4)^3`. 
...
Table: **TABLE 3.3** List of mathematical operations available in a spreadsheet.

The last operation in Table 3.3 is a function that takes the square-root of anything within the parentheses.
Other functions are also available that can make calculations across cells (e.g., `=SUM` or `=AVERAGE`).
...
```{r, echo = FALSE, fig.alt = "A screenshot of a spreadsheet with the H2 cell selected.", fig.cap = "Dataset of wasp morphological measurements from five species of fig wasps collected from Baja, Mexico in 2010. Head volume (column H) has been calculated for row 2, and to calculate it for the remaining rows, the small black square in the bottom right of the highlighted cell H2 can be clicked and dragged down to H27.", out.width="100%"}
...
$$V_{\mathrm{thorax}} = \frac{4}{3}\pi \left(\frac{Thorax_{\mathrm{L}}}{2}\right)\left(\frac{Thorax_{\mathrm{W}}}{2}\right)^{2}.$$
In the equation above, $Thorax_{\mathrm{L}}$ is thorax length (mm) and $Thorax_{\mathrm{W}}$ is thorax width. 
Substitute $Abdomen_{\mathrm{L}}$ and $Abdomen_{\mathrm{W}}$ to instead calculate abdomen volume ($V_{\mathrm{abdomen}}$).
What formula will you type into your empty spreadsheet cell to calculate $V_{\mathrm{thorax}}$? 
...
[^wasp_morphology_data]: [https://bradduthie.github.io/stats/data/wasp_morphology_untidy.xlsx](https://bradduthie.github.io/stats/data/wasp_morphology_untidy.xlsx)
