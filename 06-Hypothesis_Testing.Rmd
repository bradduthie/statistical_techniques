...
```{r, echo = FALSE, eval = TRUE, caption = "Test scores from 10 students (S1-S10) for 2 different tests in a hypothetical biology education example."}
...
```{r, echo = FALSE, eval = TRUE, caption = "Test scores from different students across 2 years, and the overall rank of each test score, in a hypothetical biology education example."}
...
This lab focuses on applying the concepts about hypothesis testing from [Chapter 20](#Chapter_20) and the statistical tests from [Chapter 21](#Chapter_21) in Jamovi.
...
First we will consider test data from 2022 ([student_scores_year_1.csv](https://raw.githubusercontent.com/bradduthie/statistical_techniques/main/data/student_scores_year_1.csv)).
...
Underneath **Hypothesis**, change the test value to 60.1 (because $\mu = 60.1$) and put the radio button to '< Test value'  to test the null hypothesis that the mean overall score of students is the same as $\mu_{0}$ against the alternative hypothesis that it is less than $\mu_{0}$.
...
```{r, echo = FALSE, fig.alt = "Jamovi interface is shown a new computed variable being made and the natural log of Test 3 scores being inserted into the funciton box.", fig.cap = "The Jamovi interface for creating a new computed variable that is the natural logarithm of Test 3 data scores in a fictional dataset of student grades.", out.width="100%"}
