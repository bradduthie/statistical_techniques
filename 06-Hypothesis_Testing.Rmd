...
A sufficient condition to fulfil this assumption is if the sample data are normally distributed [@Johnson1995; @Lumley2002].
...
Below the boxes, find the checkbox options under '**Asumption Checks**'. 
...
The Shapiro-Wilk test tests the null hypothesis that the dependent variable is normally distributed (note, this is *not* our t-test yet; we use the Shapiro-Wilk to see if the assumptions of the t-test are violated nor not).
...
Underneath **Hypothesis**, change the test value to 60.1 (because $\mu = 60.1$) and put the radio button to '< Test value'  to test the null hypothesis that the mean overall score of students is the same as $\mu$ against the alternative hypothesis that it is less than $\mu$.
