...
| **Lectures**    | 10.1: Regression key concepts (15:49 min; [Video](https://stirling.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=c59eb428-9334-4f02-bafc-af8200d91f79))     |
...
als <- read.csv("data/syconia_dimensions_raw.csv");
use <- which(is.na(als[["Vol"]]) == FALSE & is.na(als[["Lat"]]) == FALSE);
syc <- als[use,];
lat <- as.numeric(tapply(X = syc[["Lat"]], INDEX = syc[["Site"]], FUN = mean));
vol <- as.numeric(tapply(X = syc[["Vol"]], INDEX = syc[["Site"]], FUN = mean));
...
plot(x = lat, y = vol, pch = 20, xlab = "Latitude", cex = 1.5, lwd = 2,
     ...
     ylab = expression(Fruit~volume~(mm^2)), yaxs = "i", xaxs = "i");
...
text(x = 0.5, y = 3, labels = expression(R^2==0.14), cex = 1.5, pos = 4);
...
text(x = lat[1] - 0.5, y = 2100, labels = vol_devs[1], pos = 4, col = "blue");
...
Since a multiple regression investigates the effect of an independent variable in the context of other independent variables, we might sometimes expect regression coefficients to be different from what they would be given a simple linear regression [@Morrissey2018].
...
lat <- as.numeric(tapply(X = syc[["Lat"]], INDEX = syc[["Site"]], FUN = mean));
vol <- as.numeric(tapply(X = syc[["Vol"]], INDEX = syc[["Site"]], FUN = mean));
alt <- as.numeric(tapply(X = syc[["Alt"]], INDEX = syc[["Site"]], FUN = mean));
lat <- round(lat, digits = 1);
vol <- round(vol, digits = 1);
alt <- round(alt, digits = 1);
alt <- c(alt[4], alt[3], alt[1], alt[6], alt[2], alt[5], alt[7]);
Latitude   <- lat;
Volume     <- vol;
Altitude   <- alt;
...
plot(x = lat, y = vol, pch = 20, xlab = "Latitude", cex = 1.5, lwd = 2,
     ...
     ylab = expression(Fruit~volume~(mm^2)), yaxs = "i", xaxs = "i");
par(mar = c(5, 2, 1, 2));
plot(x = alt, y = vol, pch = 20, xlab = "Altitude", cex = 1.5, lwd = 2,
...
In [Chapter 31](#Chapter_31), we found that a simple regression of latitude against fruit volume had a regression coefficient of $b_{1} = -65.26$, but this was not significant ($P = 0.22287$).
...
When biomass is burned, a large proportion of its stored carbon is emitted into the atmosphere in the form of carbon dioxide, but some of remains sequestered in the soil due to incomplete combustion [@Santin2016].
This pyrogenic organic carbon can persist in the soil for long periods of time and have positive effects on soil properties [@Reisser2016].
...
We will use the [fire_carbon.csv]() dataset (right click and “Save Link As…”, then save it with the extension ‘.csv’).
...
Remember from [Chapter 31](#Chapter_31)
